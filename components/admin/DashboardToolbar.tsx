...
export default function DashboardToolbar({ total }: { total: number }) {
  const router = useRouter()
  const params = useSearchParams()

  const exportUrl = `/api/admin/inquiries/export?${params.toString()}`

  return (
    <div className="flex items-center justify-between mb-4">
      <div>
        <h1 className="text-xl font-semibold">Inquiries</h1>
        <p className="text-xs text-zinc-500">{total} matching</p>
      </div>
